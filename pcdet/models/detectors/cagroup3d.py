...
class CAGroup3D(Detector3DTemplate):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.affine_rpn = AffineLayer(num_channels=64, bias=True)
        self.affine_rcnn = AffineLayer(num_channels=64, bias=True)
    ...
    def forward(self, batch_dict):
        ...
        for i, cur_module in enumerate(self.module_list):
            ...
            if i == 0:
                results['sp_tensor'] = ME.SparseTensor(
                        coordinates=results['sp_tensor'].coordinates,
                        features=self.affine_rpn(decouple_layer(results['sp_tensor'].features, 0.)),
                        tensor_stride=results['sp_tensor'].tensor_stride
                    )
            elif i == 1:
                results['middle_feature_list'][-1] = ME.SparseTensor(
                        coordinates=results['middle_feature_list'][-1].coordinates,
                        features=self.affine_rcnn(decouple_layer(results['middle_feature_list'][-1].features, 0.75)),
                        tensor_stride=results['middle_feature_list'][-1].tensor_stride
                    )
